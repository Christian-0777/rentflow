...
#### Modified Table: `payments`
 - added nullable `due_id` INT column (FK -> dues.id)
 - indexed `due_id` for faster lookups when showing the last payment/status
 - existing rows are left NULL; new code populates the field for every insert
...
**Behavior:**
1. mark the current due record as paid (so it moves out of the "next due" column)
2. Add unpaid amount to arrear_entries with source='marked_not_paid'
3. Insert "Marked as Not Paid" payment record (the new `due_id` column links back to the original due)
4. Update total_arrears in arrears table
5. Insert next due if provided

> 📝 **Bug fix notes:**
> 1. Older implementation left the due unpaid, causing the same row to appear again after refresh
>    and the payments table would never show the unpaid amount in the previous payment column. By closing the
due and joining the new `due_id` field when calculating the last payment amount, the unpaid value now
appears correctly and is excluded from revenue aggregates.
> 2. Form-token logic was inverted – the script previously rejected the first submission (the token in
>    the form always matched the session) which meant payments were silently ignored and users had to
>    refresh twice. The check is now `===` to validate and the token is cleared after use to prevent
>    duplicates on refresh/back-navigation.
