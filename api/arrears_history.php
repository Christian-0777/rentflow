 ...
 * Returns arrears history for a lease (penalties and unpaid dues)
 */

require_once __DIR__ . '/../config/db.php';
require_once __DIR__ . '/../config/auth.php';

// Require admin or treasury role
...
try {
    ...
    // Get current arrears information
    $arrearsInfo = $pdo->prepare("
        SELECT 
            total_arrears,
            previous_arrears,
            triggered_date,
        ...
        WHERE lease_id = ?
    ");
    $arrearsInfo->execute([$leaseId]);
    $currentArrears = $arrearsInfo->fetch(PDO::FETCH_ASSOC);
    ...
    echo json_encode([
        ...
        'total_unpaid_dues' => array_sum(array_column($dues, 'amount')),
        'current_arrears' => $currentArrears ? [
            'total' => (float)$currentArrears['total_arrears'],
            'previous' => (float)$currentArrears['previous_arrears'],
            'triggered_date' => $currentArrears['triggered_date'],
            'trigger_reason' => $currentArrears['trigger_reason']
        ] : null
    ]);
