...
// Mark nearest due (lookup first so we can reference due_id when inserting)
$due = $pdo->prepare("SELECT id, amount_due FROM dues WHERE lease_id=? AND paid=0 ORDER BY due_date ASC LIMIT 1");
$due->execute([$leaseId]);
$d = $due->fetch();

// Insert payment and associate with the due if we found one
$pdo->prepare("INSERT INTO payments (lease_id, due_id, amount, payment_date, method, transaction_id, remarks) VALUES (?,?,?,CURDATE(),?,?,?, 'Recorded')")
    ->execute([$leaseId, $d['id'] ?? null, $amount, $method, $txnId]);
