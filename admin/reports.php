...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['export']) && $_GET['export'] === 'revenue_csv') {
    ...
    $revenue_data = $pdo->query("
        SELECT 
            ...
            (SELECT SUM(amount) FROM payments WHERE DATE(payment_date) = DATE(p.payment_date) AND remarks <> 'Marked as Not Paid') as total_collected,
...
$stats = $pdo->query("
    SELECT 
        SUM(CASE WHEN p.remarks <> 'Marked as Not Paid' THEN p.amount ELSE 0 END) as total_revenue,
        SUM(CASE WHEN p.remarks <> 'Marked as Not Paid' THEN p.amount ELSE 0 END) as total_collected,
