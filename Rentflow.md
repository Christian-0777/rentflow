...
5. **Role-Based Dashboard** → Redirected to role-specific dashboard:
   ...
   - Treasury → `/treasury/dashboard.php` (optional role)
...
### 3. **Treasury Role** (Optional)
- Financial oversight and adjustments
- Dashboard for financial metrics
- Specialized access for treasury operations

**Treasury Pages:**
- `/treasury/dashboard.php` - Financial overview
- `/treasury/adjustments.php` - Manual adjustments
...
### 1. **User Management**
- ✅ User registration with role assignment (Admin, Tenant, Treasury)
...
require_role(['admin', 'treasury']);  // Multiple roles
```
...
3. **Admin/Treasury Login:**
```php
admin_treasury_login($pdo, $role, $code1, $code2, $code3)
```
...
phone               VARCHAR(20)
business_name       VARCHAR(255)  -- For tenants/treasury
role                ENUM('admin','tenant','treasury') NOT NULL
